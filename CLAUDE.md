...
- **Playtesting happens in a standalone build, never the editor.** Editor overhead (domain reloads, inspector repaints, the profiler) contaminates any judgement about feel or framerate. After a gameplay change, rebuild `Builds/Win64/RogueMonk.exe` via the MCP bridge (`manage_build`, target windows64, development build, windowed) and let the human run the exe. `Builds/` is gitignored. Editor Play Mode is for Claude's automated verification only.
