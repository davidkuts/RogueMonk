...
- **Editor feedback loop:** the Coplay unity-mcp bridge is installed — prefer it to read the Unity console, inspect scenes, create/modify GameObjects, and run tests, instead of asking the human to copy-paste. Fallback when the bridge is down or the editor is closed: `Unity -batchmode -quit -projectPath . -logFile -` to check compilation, `-runTests -testPlatform EditMode` for tests, or read Editor.log. Never declare a task done on unverified code.
