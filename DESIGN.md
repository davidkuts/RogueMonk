...
- ⚠️ **Tick poise and tick armour damage are the same number** (`PoiseSystem.ApplyPoiseDamage` spends `amount` against armour first, then poise). So "the spin should never stagger" cannot be expressed by zeroing `tickPoiseDamage` without also deleting the spin's armour chip, which this document promises Armored enemies above. **Open for the human** — the honest fix is separate poise and armour numbers, not a zeroed field.
