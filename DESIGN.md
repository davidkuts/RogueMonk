...
**Recharge — currently switched off (2026-08-08 playtest).** The cooldown is set to **0**: the vortex is spammable, its damage is small (3 ticks × 1.5 = 4.5, well under half a punch), and **the pull-immunity window is what governs it instead of the cooldown**. A target that has just been dragged in cannot be pulled *or* staggered again for 1 s — it still takes the ticks, because it is standing in a vortex — which is what stops spam becoming a permanent stun-lock. The recharge machinery below is intact and re-enabled by putting a non-zero number back in the asset; it is the model to return to if "spammable" stops being fun.
