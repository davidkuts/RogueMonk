...
1. ✅ **RESOLVED 2026-08-09 — amber was triple-booked.** The theme reserves amber-gold for the Armored tier "everywhere, no exceptions", while the telegraph grammar assigned **amber = incoming projectile** and the perfect-dodge trail plus the Riposte spark are **gold**. `ENEMIES_BIOME1.md` §1 settled it by declaring amber a *semantic channel* — solidified time: armour plates, stall zones, boss armour patches — and barring it from staggerable identity colours. **The projectile telegraph moved to magenta** (human call, 2026-08-09). The armour colour is load-bearing narratively; the projectile hue was one value in two assets.
   - **Two further collisions surfaced when §1 was read against this file, both settled the same day.** ⚠️ §1 assigns **Sailspit violet**, but violet is locked here as the *gap-closer* telegraph — and §1's own rule says identity colours must stay out of reserved channels. Cerashorn's charge and Twice-Struck's horn rush both fire violet telegraphs in rooms Sailspit stands in. **Sailspit's identity moved to a muted plum**; saturated violet stays the gap-closer, because three enemies in this biome use it against one identity. And §1 reserves **cyan-white for the Twice-Struck echo**, which is the locked dash hue — the Second Hand's own colour. **The echo shifted to a pale bone-cyan**, close enough to read as the same phenomenon, far enough that "mine" and "its" never blur. The dash blue itself was not re-opened.
...
- **Obstacle phasing (added 2026-08-17):** the Blink carries the player through a marked small prop (e.g. a rock) if the dash has enough distance left to actually clear it — the object's far edge, plus the player's radius, plus a small authored leeway (`DashSettings.ObstaclePhaseLeewayMeters`, default 0.3 m). If the dash cannot clear it, the prop blocks like any other obstacle. **Walls are excluded on principle, not just by omission** — the same room-containment rule that keeps `dashPhasesThroughLayers` off walls for enemies. Marked per-object via a `DashPhaseable` component (a component, not a layer — floor/walls/props all share the Default physics layer and several systems already key off that), so phasing is opt-in per prop rather than a blanket rule.
...
- **Damage is a tick clock, and a tick strikes each BODY once — never once per collider** (locked 2026-08-12, M21.1). The maximum any single enemy can take from one cast is `tickCount × tickDamage`, **whatever shape it is**. This is not a detail: the vortex originally resolved a hit per overlapping collider, so a multi-part enemy took a multiple of the intended damage — the Tyrant's six colliders meant **18 resolves per cast instead of 3**, along with six times the poise, sparks and hitstop requests. Spamming the spin at a large single target therefore beat the combo outright, which inverts this ability's entire purpose. Every other attacker in the game already deduplicated per `IDamageable` and skipped the root capsule of a zoned body; the vortex was the sole exception, and now is not.
  - **Which plate a radial pull hits is answered by "nearest to the centre".** A swung attack lands where it was aimed, so positioning decides what it strikes; a radial pull has no aim, and taking whatever the physics query returned first would make armour apply intermittently — which reads as bad luck rather than as a rule. Nearest is deterministic and says something true: the drag bites on the part closest to the drain. A practical consequence, and the intended one: because plates surround an Ambershell, the spin usually lands on amber and reads as weak against it.
  - Enemies entering mid-channel take only the remaining ticks and enemies leaving forfeit the rest, for free — ticks are counted as they come due rather than accumulated per target, so "arrived late" needs no special case.
...
**Recharge — switched off, and being removed for good (human call 2026-08-12).** The cooldown is **0** and is not coming back: the vortex is spammable by design, and **the pull-immunity window is what governs it instead**. A target that has just been dragged in cannot be pulled *or* staggered again for 1 s — it still takes the ticks, because it is standing in a vortex — which is what stops spam becoming a permanent stun-lock. Now that per-cast damage is capped at `tickCount × tickDamage` regardless of target size, spam is bounded by the tick clock rather than by a timer.

- **The vortex therefore has no cooldown-reduction feed and must not grow one.** Per-hit CDR was deliberately never wired to the spin's own ticks ("or it would refund its own cooldown out of the crowd it just gathered"), and with the cooldown going away the question is closed. The `VortexAbility` machinery, `perHitRefundSeconds`, and the Wound Spring Stopgap that refreshes it are all **dead weight pending deletion** — kept only so the removal is one deliberate pass rather than scattered.
- The old recharge model is recorded below for history. It is no longer the model to return to.

**Recharge (retained, currently inactive).** Baseline cooldown ~10 s; every landed player hit shaves ~0.4 s; whiffs shave nothing. Aggressive play cycles it roughly twice as fast as passive play, and passive play still gets it on a fair timer. It is **explicitly not** tied to perfect dodges: a perfect dodge already pays three rewards (charge refund, Focus, Riposte), and a fourth would overload one input and make the entire kit degrade at once for players who struggle with timing — the exact frustration this ability exists to prevent. Vortex uptime rides the accessible skill axis; perfect dodging rides its own. HUD is one radial dial beside the dash pips, and readiness must be perceivable without looking at it (dial glow plus the game's signature **tick**) or players will sit on it.
...
- **The turn itself is driven from code, not from the clip** (decided 2026-08-08, second playtest). Unity's humanoid root-rotation baking delivered only about **half** of the clip's authored 366° to the body — measured at ~192° — so the circle visibly never closed, whatever the clip contained. The clip's rotation is now drained to root motion (and discarded, root motion being off), and `PlayerVortex` rotates the **character model** by an exact `spinRevolutions × 360°`. This makes the circle exact by construction, gives its direction one explicit source, and leaves the clip doing what it is actually good at: the limbs. It rotates the *visual* only — facing, aim and the hitbox never see it.
  - **The turn spans the whole move, and is applied in `LateUpdate`.** Ending it with the active window left the body still for the whole of recovery while the limbs kept moving, which reads exactly like the animation freezing part-way through. And applying it in `Update` let the Animator — which poses the character *after* Update — overwrite it, leaving a residue that accumulated several degrees off rest and kept creeping. Procedural rotation layered over animation must come after the animation.
- **The player is not rooted during the spin** (`moveSpeedMultiplier` 0.6). Rooting made a 0.63 s move feel far longer than it was, and a mobile spin suits an ability whose job is spacing rather than damage.
...
- **Three layers sit on top of the smear** (built 2026-08-12, M21B), all in the reserved dash hue and all tuned from one `VortexVfxSettings` asset kept separate from the gameplay definition: **arm trails** on the hands (the feet already carry the smear; doubling up there only thickens one ring), a **spiral ground disc** at ~55% of the pull radius — visibly inside the smear so the two never compete, the smear saying *how far* and the disc *what it is doing* — and **≤32 pooled streaks** spiralling inward on a golden-angle placement rather than an RNG draw, so no effect can ever touch seed reproducibility.
  - **The disc feeds on hits, and that is the point of the layer.** Every enemy damaged by every tick brightens it, pushes its colour toward the hot core and kicks its scroll speed. Pulses **accumulate rather than retrigger**, so a spin catching a crowd is unmistakably fiercer than one catching a single body, and they decay linearly back to a subtle rest.
  - ⚠️ **All of it is alpha-blended, never additive.** Additive was tried and lost against the light arena floor (M8), which is why `Monk/Smear` and `Monk/Telegraph` both say so in their headers. This is a standing rule for any new ground or trail effect in this game.
- **The smear is traced by the feet, not stamped from the body** (decided 2026-08-08, third playtest). Whole-body afterimages are right for the dash, where the body *travels*; on a stationary spin every ghost lands on the same spot and piles into a blob in the middle. The only thing actually sweeping the circle is the leg, so ribbons trail from both feet (`Monk/Smear`, `VortexSmear`) — which is also how the genre draws spin kicks, and it makes the arc of the attack legible rather than merely decorative. The dash keeps its body ghosts; the two effects are now distinct, which is a bonus rather than a cost.
...
- ⚠️ **Clip collision with the Riposte.** Two spins that look alike would blur the game's two most distinct buttons. They must differ on silhouette: the Riposte is a **directional** sweep committing forward through one target, the vortex is **stationary and symmetrical**, going nowhere.
  - **Interim, 2026-08-08:** with `Right Hook.fbx` containing no animation, the Riposte was falling back to the combo's Kick — the worst option, since it is literally a move the player already throws. It now uses **"Riposte Sweep"**, a second clip cut from the same Hurricane Kick take at frames 6–13 (**183°, one half-turn**) with its root rotation left baked. Half a sweep against the vortex's full stationary circle keeps the two readable apart, and it is strictly better than reusing a combo clip. **This is still a placeholder** — a proper committed counter-attack clip is the real answer.

**Playtest knobs** (all ScriptableObject, never hardcoded). **Shipped values after the 2026-08-08 pass**, with the original first draft in brackets where it changed: radius 4 m · inner ring 1.5 m (inside kick range, outside body-block jank) · **100/350/180 ms** phases *(was 120/450/250 — the whole move read as too long)* · **3 ticks × 1.5 damage** *(was 3)*, total 4.5 · **4 poise per tick** *(was 8)* · arrival stagger 0.4 s · **cooldown 0** *(was 10 s)* · **per-hit refund 0** *(was 0.4)* · pull-immunity 1.0 s, now the primary governor rather than a juggle guard.

- **The pull impulse must be matched to the target's knockback damping.** An impulse decays to a travel distance of `impulse ÷ damping`, so `impulsePerMeter = damping` (both 8) makes a target travel exactly its overshoot and arrive *on* the ring. Getting this wrong is not subtle: it drags them past the ring into the player's own capsule and they body-block at the sum of the two radii (0.777 m), which is precisely the jank the ring exists to prevent.
- **One impulse per target per spin, not one per tick.** Three stacked impulses overshoot for the same reason — each tick re-reads a distance the target has not finished travelling yet, and adds a fresh impulse on top of the velocity still carrying it. The ticks are the damage; the pull is a single delivery.
- **The spin contributes NOTHING to stagger: `tickPoiseDamage` is 0** (human call, restated 2026-08-12 — "the spin does not stagger, the combo does"). It no longer breaks poise, and no longer resets the poise regen delay either. The combo's punches and kick are the stagger tools, and ATK/SPLIT hits still cancel wind-ups.
  - ⚠️ **This also switched off the vortex's armour chip, and that is only harmless today.** `PoiseSystem.ApplyPoiseDamage` spends one `amount` against armour first and poise second, so the two cannot be separated with a data change. It costs nothing right now because **no Armored-tier enemy is reachable in a run** — `RangedSkirmisher` is the only one in the project and it is not in `LevelGenerationSettings.archetypes`. **The moment an Armored enemy is registered, this document's "Armored: takes ticks and armour damage but resists the pull" becomes false**, and the fix at that point is a separate `tickArmorDamage` on the definition plus a two-argument `ApplyPoiseDamage`. Do not discover this by shipping an amber enemy the vortex cannot scratch.
...
- **Armour has two granularities, and they compose** (added 2026-08-09, M14 Phase 0). The tier above is a bar on the **whole enemy**: strip it once and the enemy is tier 1 forever. That is right for a fully-plated archetype and is unchanged. It cannot describe an **Ambershell**, whose skull and dome are amber while its tail base and underside are not — the same enemy answering the same swing differently depending on where the player stood. So a second granularity sits beside it: a **`DamageZone` per collider**, carrying its own damage reduction and its own refusal to be staggered. An enemy may have both, either or neither.
  - **Intact amber refuses poise damage rather than absorbing it.** A plated zone beaten on forever must never accumulate toward a stagger, or "no stagger" quietly becomes "a slower stagger".
  - **Zone reduction lands target-side, after every attacker-side modifier.** A plate reduces whatever actually arrived. Boons are wired on the attacker's resolver, so there is no target-side pipeline stage for it to sit in anyway.
  - **Cracking is a timer on the zone, not a second component**, so "cracked" and "intact" cannot drift apart from what hit resolution reads. A cracked plate reports itself as ordinary flesh — which is exactly what "temporary weak point" has to mean — and re-hardens on its own.
  - **A zoned body resists the Undertow's pull while any plate is up**, at any tier. What holds an Ambershell in place is the amber on its back, and that is a collider rather than a number. Same rule as the bar, same reason.
...
- **Hue assignments** (locked 2026-08-07, amended 2026-08-09 and 2026-08-12): **red** = melee arc or burst · **magenta** = incoming projectile *(moved off amber 2026-08-09; see Theme § conflicts)* · **violet** = gap-closer, "it is coming to you" · **lime-yellow** = ground hazard, "this floor is about to hurt" · **amber** = solidified time: armour plates, stall zones, boss armour patches — the player's rule is "that blocks stagger or it blocks movement" · **pale bone-cyan** = temporal echo · **deep venom green** = lingering toxic ground that slows *and* damages.
  - **Venom vs lime-yellow is a timing distinction, not a taste one** (added 2026-08-12). Lime-yellow is a **delayed** hazard that resolves once and is answered by stepping out before it fires — the Eruption, the junk-rain. Venom is **already resolved** and stays dangerous for as long as it lasts, so it is answered by not walking back in. Same warning, opposite clock, which is why they cannot share a hue.
  - **Venom took Sailspit's goo off amber** because the mechanic outgrew the channel, not because anything was mis-coloured. The puddle was amber while it only slowed, and that was honest. Adding a damage-over-time made it a floor that hurts, and the player must never have to learn that one amber patch is safe to stand in and another is not.
  - ⚠️ **Green is the most crowded region of the wheel here**: Nature's element FX and Fray's boon identity are both bright mid-greens. Venom is deliberately darker and bluer than either, and neither of those two ever paints the floor — but a new green must be checked against all three, not just against lime-yellow. Violet arrived with the boss's Slam and lime with its Eruption, each so a new threat class could never be confused with one already learned. A centred burst (the boss's Nova) deliberately stays red — it is still a melee threat, and the ground decal already distinguishes a disc around the attacker from an arc in front of it.
- **The grammar is enforced in one asset, not by convention** (added 2026-08-09, M14 Phase 0). Every attack declares a `TelegraphChannel` — what its tell *means* — and `TelegraphPalette` holds what each channel looks like. A raw `Color` per asset made the rule unenforceable: twenty assets each carried their own idea of red and nothing stopped a new enemy inventing a hue that already meant something else. Retuning "incoming projectile" is now one field. `Custom` is the default so every attack authored before the palette keeps exactly the colour it had.
...
- **NO CEILINGS — EVER. Project-wide, all biomes, all abilities, all level design** (locked in `ENEMIES_BIOME1.md` §1, recorded here 2026-08-09 at that document's explicit instruction). Every arena and room is open-sky. No enemy ability, boss phase or level mechanic may depend on ceiling, rim or overhang geometry. Telegraphed air attacks are allowed, but every vertical threat must be **sky-sourced** — it falls from above the camera or materialises mid-air — and **ground-telegraphed** with an impact circle on the floor. The Tyrant's junk-rain is the first thing built to this rule: the debris phases into existence rather than shaking loose from rock, which is also the better lore.
- **CAMERA-FACING (SOUTH) WALLS ARE AUTHORED SHORTER than the other three, wherever the biome fiction allows** (added 2026-08-12, M22). Same category as the no-ceilings rule above: an authoring constraint that keeps a shader fix from having to fire constantly. The camera sits south of the room at a ~50° pitch, so the south wall is the only one that can stand between the player and their own body — or worse, between the player and an enemy's wind-up, which is a telegraph the game promised could be read. **M22's occlusion fade dissolves whatever is in the way**, and it works; but an effect that triggers every time the player backs up is a permanent visual cost paid for an authoring choice. At the current 3 m wall height and 50° pitch, occlusion begins about **1.7 m** from a wall. A 1.5 m south wall roughly halves that band; a 1 m one all but closes it. Height still reads as an arena boundary from above — the camera never sees the wall's face square-on.
- **Wave spawns** within rooms
- Doors gate until room cleared; level complete when final room cleared
- **Attack tokens cap how many enemies may swing at once** (added 2026-08-09, M14 Phase 0; the Hades pattern). Without one, "how hard is this room" is whatever the wave composition happened to total, and eight enemies in range means eight simultaneous wind-ups — unreadable, and unfair in a game whose whole promise is that every attack can be read. Melee and ranged are capped **separately**, because two archers plus two rushers is a legible fight where four rushers is a pile-on. A **per-archetype cap** sits on top so one type cannot monopolise the global budget; ENEMIES_BIOME1.md §2.1's "at most 2 Swiftjaws attacking regardless of pack size" is that number, living on the Swiftjaw asset rather than in raptor code. An enemy denied a token keeps circling — a pack that freezes while it waits its turn reads as broken.
